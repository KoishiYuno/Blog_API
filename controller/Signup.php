...
if(!isset($jsonData->name)||!isset($jsonData->username)||!isset($jsonData->password)||!isset($jsonData->email)){
    throwErrorMessage("The JSOn object misses required parameter",400);
    exit();
}

//vaLidate parameters
if(strlen($jsonData->name) < 1 || strlen($jsonData->name) > 255 || strlen($jsonData->password) < 8 || strlen($jsonData->password) > 255|| strlen($jsonData->email) > 255){
...
try{
    ...
    $query = $readDB->prepare('select id from jwt_user where username = :username');
    ...
    if($count !== 0){
        throwErrorMessage("The username has already exist",404);
        exit();
    }

   //check if the email is exit in user table
   $query = $readDB->prepare('select id from jwt_user where email = :email');
   ...
   //throw error message for no rows are found in database
   if($count !== 0){
       throwErrorMessage("This email address has already exist",404);
       exit();
   }
    ...
    $query = $readDB->prepare('insert into jwt_user (name, username, password, email) values (:name, :username, :password, :email)');
